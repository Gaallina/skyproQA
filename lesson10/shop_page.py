...
class ShopPage:
    ...
    def authorisation(self, name: str, password: str) -> None:
        self._driver.find_element(By.CSS_SELECTOR, "#user-name"
                                  ).send_keys(name)
        self._driver.find_element(By.CSS_SELECTOR, "#password"
                                  ).send_keys(password)
    ...
    @allure.step("Проверка добавленных в корзину товаров")
    def add_items_to_cart(self, name_item1: str, name_item2: str,
                          name_item3: str) -> None:
        """
    ...
    @allure.step("Расчет итоговой суммы товаров")
    def get_result_price(self) -> str:
        """
        Данный метод вычисляет итоговую стоимость выбранных
